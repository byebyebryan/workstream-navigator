# Historical Workstream Navigator roadmap through D18 design

> Archived 2026-08-29 during D18 authority slimming. This file preserves the
> prior roadmap verbatim below this provenance note. It is evidence, not the
> current delivery authority; see [`roadmap.md`](roadmap.md).
...
D18 current-only consolidation is implemented through D18.2 in the source
candidate, and its semantic D18.3 repository gates are implemented. D18.3 is
blocked by the falsified unprivileged online root-move proof. D17.1/schema 14
remains the installed product. The source implementation does not authorize
offboarding the current Runtimes, removing the observer, moving the current
state root, or changing the installed binary.

### D18 current-only consolidation — implementation candidate

**Status:** D18.0-D18.2 implemented and locally validated 2026-08-29. Semantic
D18.3 repository gates are implemented; reset, rollback, install, and live
acceptance are blocked by the root-move proof falsification.

D18 is one behavior-preserving checkpoint with a deliberate schema-15 reset
epoch. It removes the D16/D17 transition and compatibility plane, gives current
behavior semantic names and module boundaries, and leaves the D17.1 user
surface unchanged. It does not migrate or adopt schema-12/schema-13/schema-14
state. The authoritative contract is
[D18 current-only consolidation](design.md#current-schema-15-consolidation-boundary).

Implementation establishes the new current-state boundary first, then deletes
compatibility before broad source movement. Each slice is a separately
reviewable coherent commit, but no partial slice is an install candidate and
the checkpoint remains incomplete until the final repository and explicitly
authorized live gates pass.

#### D18.0 — Direct current-state epoch

**Implementation status:** source complete; direct schema-15 creation,
current-format bootstrap recovery, and non-mutating retired-root refusal pass
the disposable automated matrix.

- Define schema 15 as the sole accepted host schema and create it directly
  from one authoritative schema definition when the root is absent or an exact
  current-user-owned private empty directory. Publish one fixed WSNav SQLite
  application ID (`0x57534e56`, ASCII `WSNV`) plus `user_version = 15` in the
  main file before current open.
- Add the current-format stable `bootstrap.lock` ownership/recovery protocol
  with the closed `root_reserved`, `database_create_reserved`,
  `database_owned`, `database_ready`, `provisional_pending`, and `ready` graph.
  Bind root/lock and phase-specific database/provisional identities, then
  install `provisional.lock` through it with complete crash injection. Never
  create/adopt the bootstrap lock in a non-empty root; current roots must supply
  and acquire their exact existing lock before a schema-15 open.
- Open exact ready schema-15 roots and resume only unambiguous interrupted
  current-format bootstrap. Refuse schema 12-14, future/malformed/mixed roots,
  `client.sqlite`, `transition.lock`, legacy-shaped top-level evidence, and
  foreign or replaced artifacts before mutation. Old-root classification uses
  only bounded no-follow metadata and raw main-database schema identity; it
  never opens/recover SQLite or inspects legacy presentation processes.
- Rename the durable onboarding exec-target table semantically in the fresh
  schema; do not add a schema-14 migration or importer.
- Prove unchanged direct CLI and Navigator behavior against disposable fresh
  schema-15 roots before proceeding.

**D18.0 exit gate:** the startup matrix and every reserve/create/identity/
transaction/checkpoint/sync/rename/provisional/ready boundary pass with exact
phase-specific recovery or filesystem/database non-mutation evidence. Fresh
bootstrap uses rollback journaling and publishes no staging sidecar. Fresh state
contains only the current schema and artifacts, and all retained D17.1 flows
pass without a migration fixture.

#### D18.1 — Transition and compatibility deletion

**Implementation status:** source complete; retired open/migration/cutover,
observer-handover, short-Runtime-path, and profile-upgrade routes are deleted.

- Delete the D16 cutover/startup orchestration, schema-12/schema-13/schema-14
  migration machinery and full historical fixtures, retired client-catalog
  cleanup, and legacy presentation classification/drain/retirement. Retain
  only inert raw-header refusal samples, never an old-schema validator/opener.
- Delete D16 OpenCode observer standby/handover machinery, journals, internal
  commands, and transition-only tests.
- Delete short-Runtime-ID path compatibility and profile-schema-1 upgrade
  behavior. Retain exact full-UUID paths and the current Codex profile contract
  only; unsupported/foreign evidence remains untouched.
- Remove historical checkpoint harnesses from the current gate only after
  equivalent current-contract coverage exists. Preserve historical evidence
  documents unchanged.

**D18.1 exit gate:** no production route can mutate or adopt a pre-schema-15
root, old presentation, short Runtime path, or old profile contract. Static
inventory plus focused refusal tests prove the deleted behavior has no active
entrypoint, and the full retained lifecycle/recovery matrix remains green.

#### D18.2 — Retired surface deletion and semantic decomposition

**Implementation status:** source complete; retired Projects-browser and
arbitrary-registration surfaces are deleted, current modules use semantic
roles, and the retained matrix passes strict local lint and tests.

- Delete the retired Projects-page/browser root, refresh, arbitrary
  registration, DTO, error, schema, filesystem, and test surfaces, including
  repository inspection used only by that workflow. Retain and rename the one
  private promotion-time Project/Location/Workstream/Runtime transaction and
  contextual `n` Location reuse.
- Remove stale public aliases/exports/error variants and test-only production
  seams that have no current caller.
- Replace checkpoint-qualified D16/D17 module, type, function, hidden-command,
  environment-variable, artifact, and current-table names with semantic
  current names. Keep actual durable/provider protocol format versions and
  historical evidence versioned.
- After deletion, split state responsibilities into current schema/bootstrap,
  registry, onboarding, observer, and Project/Location projection boundaries;
  split presentation ownership/topology, control, attachment, provisional
  shell, and cleanup; and narrow provider-neutral contracts around the concrete
  Codex/OpenCode adapters.
- Keep the entrypoint thin and preserve deterministic test seams. Do not add a
  speculative provider framework or impose line-count targets.

**D18.2 exit gate:** active code and generated CLI help contain no delivery-
checkpoint compatibility names; retired APIs have no compiled production or
test-only operational surface; explicitly historical refusal fixtures and
D17.1 release tooling cannot open an old compatibility route; dependency
direction matches the design structure; and all retained behavior, bounded-I/O,
privacy, and fail-closed tests pass.

#### D18.3 — Reconciliation, release gates, and explicit reset

**Implementation status:** blocked by design falsification. Semantic current
repository gates are implemented and the authority documents record the
blocker, but final authority slimming, exact-candidate, clean Ubuntu/MSRV,
disposable-provider, reset, rollback, installed-parity, and ordinary-root gates
remain incomplete. An unprivileged online verifier cannot prove the race-free
zero-holder condition around the whole-root rename; see
[spike 0027](evidence/spikes/0027-d18-root-move-falsification.md).

- Replace the historical D12/D17 gate scripts with semantic current source,
  CLI, and disposable presentation acceptance checks.
- Reconcile the design, roadmap, documentation map, operator contract, and
  sanitized acceptance records against the final exact implementation. Slim
  the design and roadmap to current authority plus concise checkpoint/status
  indexes; move detailed D0-D17 transition and release narratives to dated
  evidence with preserved provenance instead of retaining duplicate
  present-tense compatibility instructions.
- Pass `scripts/check`, staged and unstaged `git diff --check`, the declared
  MSRV, dependency license/advisory/source policy, packaging, and equivalent
  clean Ubuntu validation.
- Record the exact D18 candidate checksum and, only after explicit operator
  approval, pass sanitized disposable Codex/OpenCode acceptance before any
  ordinary-state mutation. Staging-path execution is allowed; every root,
  repository, provider home, and tmux socket is disposable and completely
  cleaned.
- Verify the still-installed D17.1 binary against its recorded accepted hash.
  Use it to resolve all operations, park every Codex/OpenCode Runtime including
  archived Workstreams, close presentation/review/provisional state, and remove
  its exact owned Codex observer. If exact removal preserves a validated
  provider-settings-only file, require explicit operator authorization to hash
  and atomically rename the complete file to an inactive backup before fresh
  D18 setup; never adopt or import it automatically. Then atomically quarantine
  the installed D17.1 executable at its checksummed inactive rollback path and
  sync the parent, closing the ordinary command path before the root moves.
- Require sanitized D17.1 release tooling to hold the exact provisional lease
  and an exclusive no-mutation SQLite quiescence fence while proving zero
  live/unknown Runtime or observer across both providers, zero unresolved
  operation, zero presentation or materialized provisional/review authority,
  zero process executing the retired binary, and zero other process/FD/socket
  holder beneath the root. While still fenced, atomically rename the complete
  root to a new non-existing sibling on the same filesystem and sync the parent.
  This verifier is not a D18 compatibility entrypoint.
- Install the already accepted D18 artifact, verify byte identity, directly
  create the ordinary schema-15 root, and complete normal explicit observer
  trust. Any post-install disposable replay remains separate from that ordinary
  root and is completely cleaned.
- Exercise the symmetric external rollback using disposable D17.1/D18 epochs:
  exact D18 offboard and observer removal, D18 executable quarantine, fenced
  atomic D18-root backup, atomic preserved post-offboard old-root restoration
  while the command path is vacant, exact D17.1 binary restoration, and fresh
  D17.1 native trust. Cover both
  absent and explicitly backed-up provider-settings-only profile outcomes.

The old `host.sqlite` and original executable path must remain available
through observer removal because together they provide the D17.1 ownership
contract. Moving the root or replacing the binary first makes exact removal
unavailable and the remaining profile foreign to D18; D18 must refuse rather
than adopt it. If either epoch cannot prove exact whole-product offboarding,
the cutover or rollback stops. No compatibility code, partial state deletion,
copy/merge reset, or force-clean path may be added to bypass that refusal.

**D18 checkpoint exit gate:** all D18.0-D18.3 gates pass for one exact artifact;
pre-cutover live acceptance passes both providers without a prompt or turn and
with complete disposable cleanup; the D17.1 hash/offboard/observer-removal and
atomic backup proof passes; the installed D18 binary is byte-identical; the
ordinary root reaches exact schema 15 and observer readiness; the disposable
two-epoch rollback drill passes; and current operator documentation names
schema 15 as the sole active contract. Until then, D17.1 remains the supported
installed state.
...
The [V1 design](design.md#current-concrete-provider-boundary) now records a
...
[multi-provider design](design.md#current-concrete-provider-boundary) is

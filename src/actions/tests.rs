use super::{
    ...
    creation::{
        ForkActionEffects, ForkReconciliationResult, IndependentStartSpec,
        fork_workstream_with_effects, recover_managed_operation_with_effects,
...
fn fork_registry_for_provider(
    ...
    let runtime = registry
        ...
        .unwrap();
    let session = ProviderSessionId::new(provider, format!("source-{provider}")).unwrap();
    drop(registry);

    let connection = Connection::open(root.host_database_path()).unwrap();
    connection
        .execute(
            "UPDATE runtimes SET lifecycle = 'idle'
             WHERE runtime_id = ?1",
    ...
    connection
        .execute(
            "INSERT INTO provider_bindings (
             ...
             ) VALUES (?1, ?2, ?3, ?4, 'startup', ?5, 'source-thread', 'named', NULL,
                NULL, NULL, ?6, 1)",
            params![
...
enum FakeForkMode {
    CodexSuccess,
    CodexLostResultFound,
    CodexLostResultAbsent,
    OpenCodeSuccess,
    OpenCodeExternalEffectUnknown,
    OpenCodeRecoverySuccess,
}

struct FakeForkEffects {
    mode: FakeForkMode,
    fail_first_source_check: bool,
    source_checks: usize,
    ...
    operation_id: Option<crate::domain::OperationId>,
}

impl FakeForkEffects {
    fn new(mode: FakeForkMode) -> Self {
        Self {
            mode,
            fail_first_source_check: false,
            source_checks: 0,
    ...
    fn codex_destination() -> ProviderSessionId {
        ProviderSessionId::codex("codex-destination").unwrap()
    }

    fn opencode_destination() -> ProviderSessionId {
        ProviderSessionId::new(ProviderKind::OpenCode, "opencode-destination").unwrap()
    }
}

fn assert_fork_effect_boundary(
    registry: &crate::state::HostRegistry,
    plan: &crate::state::ForkPlan,
) {
    let stored = registry.fork_plan(plan.operation.id).unwrap();
    assert!(
        plan.fork_attempted_at_millis.is_some(),
        "provider effect must receive a plan after the durable attempt marker"
    );
    assert!(stored.fork_attempted_at_millis.is_some());
    assert_eq!(
        stored.fork_attempted_at_millis,
        plan.fork_attempted_at_millis
    );
    assert_eq!(stored.operation.phase, plan.operation.phase);
    assert_eq!(stored.operation.revision, plan.operation.revision);
    assert_eq!(
        stored.operation.effect_watermark,
        plan.operation.effect_watermark
    );
    assert!(
        !registry
            .workstream_overviews()
            .unwrap()
            .iter()
            .any(|overview| overview.workstream_id == plan.workstream_id)
    );
}

impl ForkActionEffects for FakeForkEffects {
    fn source_available(
        &mut self,
        _root: &crate::state::StateRoot,
        _registry: &mut crate::state::HostRegistry,
        plan: &crate::state::ForkPlan,
    ) -> Result<(), ActionError> {
        self.events.push("source");
        self.source_checks += 1;
        self.operation_id = Some(plan.operation.id);
        if self.fail_first_source_check && self.source_checks == 1 {
    ...
    fn provider_ready(
        &mut self,
        _registry: &crate::state::HostRegistry,
        _provider: ProviderKind,
    ) -> Result<(), ActionError> {
        self.events.push("ready");
        self.provider_ready_calls += 1;
        Ok(())
    }
    ...
    fn codex_fork(
        &mut self,
        _root: &crate::state::StateRoot,
        registry: &mut crate::state::HostRegistry,
        plan: &crate::state::ForkPlan,
    ) -> Result<ProviderSessionId, ActionError> {
        assert_fork_effect_boundary(registry, plan);
        self.events.push("codex_fork");
        self.codex_fork_calls += 1;
        match self.mode {
            FakeForkMode::CodexSuccess => Ok(Self::codex_destination()),
            FakeForkMode::CodexLostResultFound | FakeForkMode::CodexLostResultAbsent => {
    ...
    fn codex_reconcile(
        ...
        assert_fork_effect_boundary(registry, plan);
        self.events.push("codex_reconcile");
        self.codex_reconcile_calls += 1;
        match self.mode {
            FakeForkMode::CodexLostResultFound => {
                Ok(ForkReconciliationResult::Found(Self::codex_destination()))
            }
            FakeForkMode::CodexLostResultAbsent => Ok(ForkReconciliationResult::Absent),
            _ => panic!("Codex reconciliation was called for {:?}", self.mode),
        }
    }
    ...
    fn opencode_fork(
        ...
        assert_fork_effect_boundary(registry, plan);
        self.events.push("opencode_fork");
        self.opencode_fork_calls += 1;
        match self.mode {
            FakeForkMode::OpenCodeSuccess | FakeForkMode::OpenCodeRecoverySuccess => {
                Ok(Self::opencode_destination())
            }
            FakeForkMode::OpenCodeExternalEffectUnknown => Err(ActionError::ForkRecoveryRequired),
            _ => panic!("OpenCode effect was called for {:?}", self.mode),
        }
    }
    ...
    fn start(
        ...
        assert_eq!(overviews.len(), 2, "start must follow durable fork commit");
        assert!(
            registry.unresolved_operation_overviews()?.is_empty(),
            "start must follow the terminal operation commit"
        );
        self.start_calls.push(expected_revision);
        Ok(())
    }
}
...
#[test]
fn codex_fork_commits_before_start_and_reuses_the_request_without_a_second_fork() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::Codex);
    let mut effects = FakeForkEffects::new(FakeForkMode::CodexSuccess);

    let destination = fork_workstream_with_effects(
        &root,
        &mut registry,
        source_workstream_id,
        None,
        "codex-fork-success".to_owned(),
        &mut effects,
    )
    .unwrap();
    ...
    assert_eq!(
        registry
            ...
            .unwrap()
            .into_iter()
            .find(|overview| overview.workstream_id == destination)
            .unwrap()
            .provider,
        ProviderKind::Codex
    );

    let replay = fork_workstream_with_effects(
        &root,
        &mut registry,
        source_workstream_id,
        None,
...
#[test]
fn opencode_fork_commits_before_start_and_reuses_the_request_without_a_second_fork() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::OpenCode);
    let mut effects = FakeForkEffects::new(FakeForkMode::OpenCodeSuccess);
    ...
    let destination = fork_workstream_with_effects(
        ...
        source_workstream_id,
        None,
        "opencode-fork-success".to_owned(),
        &mut effects,
    )
    .unwrap();
    ...
    assert_eq!(
        effects.events,
        ["ready", "source", "source", "opencode_fork", "start"]
    );
    assert_eq!(effects.opencode_fork_calls, 1);
    assert_eq!(effects.start_calls, vec![Some(Revision::INITIAL)]);
    assert_eq!(
        registry
            .workstream_overviews()
...
#[test]
fn codex_fork_reconciles_a_lost_result_without_retrying_the_provider() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::Codex);
    let mut effects = FakeForkEffects::new(FakeForkMode::CodexLostResultFound);
    ...
    fork_workstream_with_effects(
        ...
        source_workstream_id,
        None,
        "codex-fork-lost-result".to_owned(),
        &mut effects,
    )
    .unwrap();
...
#[test]
fn codex_fork_absent_reconciliation_enters_recovery_and_recovers_exactly_once() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::Codex);
    let mut effects = FakeForkEffects::new(FakeForkMode::CodexLostResultAbsent);
    ...
    let operation = registry
        ...
        .unwrap()
        .into_iter()
        .find(|operation| operation.kind == crate::domain::OperationKind::Fork)
        .unwrap();
    let plan = registry.fork_plan(operation.operation_id).unwrap();
    assert_eq!(
        plan.operation.phase,
        crate::domain::OperationPhase::RecoveryRequired
    );
    assert!(plan.fork_attempted_at_millis.is_some());

    effects.mode = FakeForkMode::CodexLostResultFound;
    let destination = recover_managed_operation_with_effects(
        &root,
        &mut registry,
        operation.operation_id,
        Some(plan.operation.revision),
        &mut effects,
    )
    .unwrap();
    assert_eq!(
        effects.codex_fork_calls, 1,
    ...
    assert!(
        registry
            ...
            .unwrap()
            .iter()
            .any(|overview| overview.workstream_id == destination)
    );
    assert!(
        registry
            .unresolved_operation_overviews()
...
#[test]
fn opencode_unknown_effect_is_terminal_and_never_retried() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::OpenCode);
    let mut effects = FakeForkEffects::new(FakeForkMode::OpenCodeExternalEffectUnknown);
    ...
    assert!(matches!(
        fork_workstream_with_effects(
            ...
            source_workstream_id,
            None,
            "opencode-fork-unknown".to_owned(),
            &mut effects,
        ),
    ...
    assert_eq!(effects.opencode_fork_calls, 1);
    assert!(effects.start_calls.is_empty());
    let operation_id = effects.operation_id.unwrap();
    let plan = registry.fork_plan(operation_id).unwrap();
    assert_eq!(plan.operation.phase, crate::domain::OperationPhase::Failed);
    assert!(
        plan.operation
            .outcome_json
            .as_deref()
            .is_some_and(|outcome| outcome.contains(crate::state::EXTERNAL_EFFECT_UNKNOWN_CODE))
    );
...
#[test]
fn opencode_unattempted_recovery_records_attempt_before_effect_and_commits() {
    let (_temporary, root, mut registry, source_workstream_id) =
        fork_registry_for_provider(ProviderKind::OpenCode);
    let mut effects = FakeForkEffects::new(FakeForkMode::OpenCodeRecoverySuccess);
    effects.fail_first_source_check = true;
    ...
    assert!(matches!(
        fork_workstream_with_effects(
            ...
            source_workstream_id,
            None,
            "opencode-fork-unattempted-recovery".to_owned(),
            &mut effects,
        ),
        Err(ActionError::ForkRecoveryRequired)
    ));
    ...
    assert!(plan.fork_attempted_at_millis.is_none());
    assert_eq!(effects.opencode_fork_calls, 0);

    effects.fail_first_source_check = false;
    let destination = recover_managed_operation_with_effects(
        &root,
        &mut registry,
        operation_id,
